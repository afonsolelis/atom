...
- **Átomo 3.0** (`disciplinas/industria_4_0_e_digitalizacao_de_processos/`, `disciplinas/sistemas_de_informacao_automacao_e_ia_aplicada_a_producao/`, `disciplinas/data_engineering_and_pipelines/`, `disciplinas/portos_aeroportos_e_ferrovias/`): the current Ânima/Átomo 3.0 template — `unidade_N/unidade_N.md` (texto + roteiros + quiz + AAI + material complementar inline) + `unidade_N/questoes_uniN.md` + a shared `instrumentos_avaliativos/` folder with `avaliacao_final.md` and `entrega_trabalho.md`. **Use this layout for any new discipline**, unless the contract for that discipline specifies otherwise (see the TECH variant below).
- **Átomo 3.0 TECH variant** (`disciplinas/distributed_systems_engineering/`): same one-file-per-unit shape as Átomo 3.0 (`unidade_N/unidade_N.md` + `unidade_N/questoes_uniN.md`), but with contract-specific deviations — **read `DIRETRIZES_PRODUCAO.md` and `PLANO_APRENDIZAGEM_PROPOSTO.md` in the discipline root first, they're the source of truth, not this file**. Key differences from standard Átomo 3.0: roteiros live in a sibling `unidade_N/roteiros_20min.md` (one file with all 4 videoaula scripts, 20 min/~2.200–2.700 spoken words each) instead of inline per-aula; `questoes_uniN.md` has **40 questões** (20 asserção-razão + 20 interpretação) instead of 20, with feedback for **every alternative**, not just the correct one; the final exam is `instrumentos_avaliativos/avaliacao_dissertativa.md` — **10 discursivas only**, no objective questions; AAI (1 dissertativa + resposta esperada) appears **only in Unidade 1**; "Recurso visual N" inside `unidade_N.md` is a descriptive blockquote placeholder for the editing team (with alt text), not an embedded Wikimedia image. Root docs `CRONOGRAMA.md` (delivery checklist) and `ANALISE_MATERIAIS_RECEBIDOS.md` (documents a real 20-vs-40-questions conflict between the official templates and the contract, resolved in favor of the contract) track production status. **Slides do follow the standard Átomo 3.0 HTML deck convention** (see Slides section below) — same `assets/template_apresentacao/` pattern as `disciplinas/data_engineering_and_pipelines/`, not the PPTX originally implied by the received materials; the NEaD-supplied PPTX templates under `materiais_recebidos/` were superseded by this HTML deck decision.
- **Unit-based (legacy)** (`disciplinas/Estruturas_de_Concreto_Pilares_e_Solicitacoes_Dinamicas/`, `modelos/nova_disciplina/`): older layout — 4 units, each with `unidade_N_conteudo.md` + four `unidade_N_i_aula.md` + `questoes_unidade_N.md`.
- **Aula-based** (`disciplinas/estrutura_pontes/`, `disciplinas/engenharia_de_prompt_a_mentalidade_de_ia_e_o_conhecimento_digital/`): 16 individual `aulaN.md` files in `aulas/` plus parallel `roteiros/`, `questoes/`, `m_complementar/` directories. 16 aulas = 4 units × 4 aulas, but flattened.
...
**Ignore the agentic-tooling scaffolding** committed under `disciplinas/industria_4_0_e_digitalizacao_de_processos/` — `.aiox-core/` (~1100 files), `.antigravity/`, and the discipline-level `.claude/` are from an unrelated tool and have nothing to do with the coursework. Don't read, edit, or reason about them when authoring content. (`roteiros_video/` in that discipline is untracked local scratch — `.txt` drafts, not deliverables.)
...
`disciplinas/estrutura_pontes/diretrizes_latex.json` adds discipline-specific conventions: use `\mathrm{}` + thin space (`\,`) for units (`15\,\mathrm{kN}`), vírgula decimal (`{,}` in LaTeX), and standard sign conventions (`+` tração, `+` sagging). It also expects an `aulaN_ponte.svg` per aula. `disciplinas/engenharia_de_prompt_a_mentalidade_de_ia_e_o_conhecimento_digital/directions.json` removes "Objetivos"/time markers and expects an `aulaN_prompt_flow.svg` diagram instead.
...
Each Átomo 3.0 aula has a self-contained HTML deck at `<disciplina>/unidade_N/slides/aulaN.html` — this applies to all five disciplines that use it: `disciplinas/industria_4_0_e_digitalizacao_de_processos/`, `disciplinas/sistemas_de_informacao_automacao_e_ia_aplicada_a_producao/`, `disciplinas/data_engineering_and_pipelines/`, `disciplinas/portos_aeroportos_e_ferrovias/`, and `disciplinas/distributed_systems_engineering/` (TECH variant, decks added after the discipline's markdown content). Numbering is **continuous across units** (matching the videoaula numbering): `unidade_1/slides/` holds `aula0.html` (the unit-1-only intro deck) + `aula1.html`–`aula4.html`, `unidade_2/slides/` holds `aula5.html`–`aula8.html`, etc. — so **17 decks per discipline** (`aula0` + `aula1`–`aula16`), 85 total across the five disciplines. These are **single-file** decks — all CSS/JS inline, no build step — opened directly in a browser; export to PDF via Chrome's *Print → Save as PDF* (landscape, no margins, "background graphics" on, one slide per page).
...
Use `unidade_2/unidade_2_1_aula.md` of `disciplinas/Estruturas_de_Concreto_Pilares_e_Solicitacoes_Dinamicas/` as the reference for depth and pacing — the diretrizes explicitly name it as the model.
