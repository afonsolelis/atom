...
class Topico:
    ...
    def publicar(self, chave: str, tipo: str, payload: dict[str, Any]) -> Evento:
        indice_particao = escolher_particao(chave, self.num_particoes)
        evento = Evento(chave=chave, tipo=tipo, payload=payload, offset=len(self._particoes[indice_particao]))
