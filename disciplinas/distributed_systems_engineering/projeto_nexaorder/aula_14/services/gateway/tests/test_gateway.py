...
def test_trace_id_do_gateway_se_propaga_para_os_servicos_downstream(plataforma):
    """Unidade 4, Aula 13. Até a Aula 12, o gateway gerava (ou recebia) um
    trace_id na borda e nunca o repassava adiante — o primeiro salto já
    rompia a correlação, mesmo com pedidos/estoque/pagamento/expedicao
    todos instrumentados. Prova que agora cada serviço chamado por
    `resumo_do_pedido` registra, na própria coleção de spans, um span com
    exatamente o trace_id que o cliente enviou ao gateway."""
    cliente_pedidos, gateway, estoque, pagamento, expedicao = plataforma
    pedido = _criar_pedido(cliente_pedidos)
    cliente_pedidos.post(f"/pedidos/{pedido['id']}/finalizar-compra")
    ...
    meu_trace_id = "c" * 32
    resposta = TestClient(gateway.app).get(
        f"/pedidos/{pedido['id']}/resumo", headers={"X-Trace-Id": meu_trace_id}
    )
    assert resposta.status_code == 200

    assert TestClient(estoque.app).get(f"/_admin/spans/{meu_trace_id}").json() != []
    assert TestClient(pagamento.app).get(f"/_admin/spans/{meu_trace_id}").json() != []
    assert TestClient(expedicao.app).get(f"/_admin/spans/{meu_trace_id}").json() != []
