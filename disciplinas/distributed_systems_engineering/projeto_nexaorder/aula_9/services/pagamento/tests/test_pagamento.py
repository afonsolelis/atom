...
def test_listar_cobrancas_por_pedido(cliente_api):
    cliente_api.post(
        "/cobrancas", json={"pedido_id": "pedido-x", "chave_idempotencia": str(uuid.uuid4()), "valor": 10.0}
    )

    resposta = cliente_api.get("/cobrancas/por-pedido/pedido-x")
