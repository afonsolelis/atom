...
app = FastAPI(title="NexaOrder — Pedidos", version="0.8.0")
...
async def _liberar_estoque(reserva_id: str, trace_id: str) -> None:
    try:
        await _cliente_estoque.post(
            f"{ESTOQUE_BASE_URL}/reservas/{reserva_id}/liberar", json={}, trace_id=trace_id
        )
...
async def _estornar_pagamento(cobranca_id: str, trace_id: str) -> None:
    try:
        await _cliente_pagamento.post(
            f"{PAGAMENTO_BASE_URL}/cobrancas/{cobranca_id}/estornar", json={}, trace_id=trace_id
        )
    except (CircuitoAberto, httpx.TimeoutException, FalhaTransitoria):
        pass
