...
Um valor baixo sugere um serviço relativamente estável, muito pressionado por consumidores e pouco dependente de outros componentes. Serviços com instabilidade alta — poucas dependências aferentes e muitas dependências eferentes — sofrem menos pressão de consumidores externos, mas ficam mais sujeitos a mudanças nas dependências que consomem. A métrica não substitui julgamento de negócio nem mede criticidade: ela apenas transforma uma parte do acoplamento estrutural em um número discutível pela equipe.
...
- O princípio de dados por serviço evita acoplamento oculto por esquema compartilhado, ao custo de composição explícita entre serviços.
...
O roteiro falado completo, com narração pronta para gravação, marcações de edição e fontes, está no arquivo `roteiros_20min.md` desta unidade, usando o diagnóstico do monólito distribuído da NexaOrder como demonstração central.
...
Escolher mal a chave de particionamento compromete essa garantia. Se a NexaOrder particionasse por tipo de evento — por exemplo, `pedido-criado`, `estoque-reservado` e `pagamento-aprovado` — em vez de usar de forma estável o identificador do pedido, eventos do mesmo pedido poderiam cair em partições distintas e ser observados fora de ordem pelo consumidor.
...
Um grupo de consumidores também se reorganiza automaticamente diante de falhas, em um processo chamado **rebalanceamento**: se uma instância falha, as partições que estavam sob sua responsabilidade são redistribuídas entre as instâncias remanescentes do mesmo grupo. Se um grupo de 3 instâncias responsável por 8 partições perde uma instância, as 2 restantes passam a dividir as 8 partições entre si. A carga por instância remanescente aumenta e a capacidade total do grupo pode diminuir até que uma nova réplica seja adicionada. Esse comportamento reproduz, na camada de consumo de eventos, o princípio de redundância sem ponto único de falha discutido na Aula 1.

> **Recurso visual 5 — Tópico com partições e grupo de consumidores:** diagrama de um tópico "eventos-pedido" com 8 partições, chaves por identificador de pedido, e duas instâncias de um grupo de consumidores dividindo as partições entre si.  
...
Para a NexaOrder, essa propriedade permite, por exemplo, reconstruir o painel de métricas de vendas após uma correção no consumidor. O log de eventos só pode ser tratado como fonte de verdade quando o sistema foi deliberadamente projetado para isso — com retenção suficiente, eventos completos e imutáveis, versionamento e garantias de durabilidade; caso contrário, ele é apenas uma das fontes necessárias para a reconstrução.
...
- **At-least-once:** sob as hipóteses operacionais da plataforma — publicação confirmada em armazenamento durável, retenção vigente e retries disponíveis — o evento é entregue uma ou mais vezes; a duplicação é possível. Falhas fora dessas hipóteses, expiração da retenção ou configuração incorreta ainda podem causar perda.
- **Exactly-once:** dentro de uma fronteira declarada, cada evento produz exatamente um efeito observável, mesmo diante de reentregas. A garantia não elimina duplicações na transmissão e não se estende automaticamente a efeitos externos à fronteira transacional da plataforma. Dentro dela, normalmente combina *at-least-once* com deduplicação ou operações idempotentes no consumidor; fora dela — por exemplo, ao chamar um provedor de pagamento — exige idempotência ponta a ponta ou reconciliação explícita do resultado.

Na prática, muitas plataformas de eventos oferecem configuração *at-least-once*, deixando a responsabilidade de alcançar um efeito efetivamente único para o desenho do consumidor. Na NexaOrder, o consumidor de "pagamento aprovado" registra o identificador do evento e o efeito de negócio na mesma transação local — ou usa uma máquina de estados equivalente — antes de confirmar o consumo. Assim, uma falha não cria a janela perigosa em que o identificador foi gravado, mas a expedição ainda não foi produzida.

> **Recurso visual 6 — Três semânticas de entrega lado a lado:** três linhas do tempo comparando at-most-once (mensagem perdida), at-least-once (mensagem duplicada) e exactly-once (efeito único apesar de duplicação na transmissão).  
...
Se a NexaOrder decidir adicionar um campo `canal_venda` ao evento "pedido criado", consumidores antigos que ignoram campos desconhecidos continuam funcionando. Se renomear `valor_total` para `valor_liquido` sem transição, um consumidor não atualizado poderá rejeitar ou falhar ao desserializar o evento, aplicar um valor padrão/nulo ou tratar o campo como ausente, conforme o contrato e o validador usados. Em todos esses casos, a compatibilidade foi quebrada.

> **Recurso visual 7 — Evolução de esquema:** diagrama mostrando um evento v1 com campos base, um evento v2 com um campo opcional adicionado, e um consumidor v1 lendo o evento v2 sem erro.  
...
> **Recurso visual 8 — Fluxo de eventos do pedido:** linha do tempo com os quatro serviços da NexaOrder publicando e consumindo eventos em sequência, sem chamadas síncronas diretas entre eles.  
...
O roteiro falado completo, com narração pronta para gravação, marcações de edição e fontes, está no arquivo `roteiros_20min.md` desta unidade, usando a reorganização do fluxo de checkout da NexaOrder como demonstração central.
...
A **imutabilidade** é uma convenção central: em vez de corrigir uma instância em execução, a prática recomendada é publicar uma nova imagem e substituir os contêineres antigos por novos, criados a partir dela. Isso reduz a divergência silenciosa do artefato de aplicação entre ambientes, pois a mesma imagem pode ser promovida sem alterações manuais posteriores. Não elimina diferenças de configuração, infraestrutura, dados, arquitetura do host ou serviços externos; essas variáveis ainda precisam ser controladas e testadas.
...
Esse manifesto mínimo e válido declara o estado desejado: quatro réplicas do contêiner de imagem `nexaorder/pagamento:1.7.0`, selecionadas pelo rótulo `app: pagamento`. Se um Pod terminar ou for removido e restarem apenas três, existirá uma divergência entre o número desejado e o observado.
...
Quando um Pod do pagamento termina ou é removido na situação-problema, o controlador associado ao Deployment observa que existem apenas três dos quatro Pods desejados e cria outro para restaurar o número declarado. Se apenas o processo dentro de um Pod vivo travar e uma sonda de vivacidade falhar, o kubelet normalmente reinicia o contêiner no mesmo Pod; não é necessário criar um quinto Pod. Os dois mecanismos produzem recuperação automática, mas atuam em níveis diferentes.
...
Mas como o Kubernetes sabe, na prática, que um contêiner deixou de estar saudável? Essa percepção depende de **sondas** configuradas pela equipe responsável. Uma **sonda de vivacidade** (*liveness probe*) verifica se o contêiner ainda consegue progredir; quando falha repetidamente, o kubelet reinicia esse contêiner de acordo com a política do Pod. Uma **sonda de prontidão** (*readiness probe*) verifica se o Pod está apto a receber tráfego; quando falha, o Pod permanece em execução, mas é removido dos destinos prontos do Service. Uma **sonda de inicialização** pode proteger aplicações lentas durante a partida. Sondas devem verificar sinais úteis sem transformar uma dependência externa instável em reinícios em cascata.

> **Recurso visual 9 — Laço de reconciliação:** diagrama circular com três etapas — observar estado atual, comparar com estado desejado, agir para reduzir a diferença — retornando ao início.  
...
Contêineres imutáveis não devem embutir configuração específica de ambiente ou credenciais na própria imagem. O Kubernetes separa essas preocupações em **ConfigMaps**, para configuração não sensível, e **Secrets**, para dados sensíveis, injetados nos Pods em tempo de execução. Valores expostos como variáveis de ambiente não mudam no processo já iniciado; atualizações exigem reinício controlado do Pod. Volumes projetados podem receber atualizações de forma eventual, mas a aplicação precisa reler o arquivo. Para rotação avançada e auditoria, é comum integrar o cluster a um gestor externo de segredos.
...
> **Recurso visual 10 — Service roteando para Pods saudáveis:** diagrama de um Service estável recebendo tráfego e distribuindo entre três Pods do Deployment de pagamento, com um quarto Pod recém-substituído.  
...
Um exemplo concreto ajuda a fixar o mecanismo. Se o Deployment de pagamento possui 6 réplicas e a atualização é configurada para permitir, no máximo, 1 réplica indisponível e 1 réplica excedente durante a transição, o Kubernetes cria inicialmente 1 Pod com a versão nova — totalizando 7 Pods, 6 antigos e 1 novo —, aguarda esse Pod passar na sonda de prontidão e só então remove 1 Pod antigo, voltando a 6 no total. O ciclo se repete até que todas as réplicas estejam na versão nova, sem que a capacidade saudável caia abaixo de 5 nem ultrapasse 7 em nenhum momento. Se o Pod novo falhar repetidamente na sonda de prontidão, o avanço pode ficar bloqueado antes de substituir todas as réplicas. O Deployment não executa rollback automático por padrão: a equipe ou uma automação externa deve observar a condição de progresso e decidir pausar ou reverter a versão.

> **Recurso visual 11 — Atualização gradual:** sequência de quatro instantâneos de um Deployment substituindo réplicas da versão 1.6.0 pela versão 1.7.0, uma de cada vez, mantendo o total de réplicas saudáveis.  
...
4. Como alertas de reinício, `progressDeadlineSeconds`, políticas operacionais e automação externa poderiam interromper uma implantação defeituosa e acionar intervenção humana, considerando que um Deployment não oferece um número máximo de reinícios como Jobs oferecem com `backoffLimit`?
...
1. A partir do manifesto apresentado nesta aula, descreva separadamente o que ocorreria se dois Pods terminassem e se dois Pods apenas falhassem na readiness, mantendo-se em execução.
...
- Imagens de contêiner imutáveis reduzem a divergência do artefato entre ambientes; configuração, infraestrutura, dados e dependências externas ainda podem variar.
...
O roteiro falado completo, com narração pronta para gravação, marcações de edição e fontes, está no arquivo `roteiros_20min.md` desta unidade, usando a recuperação automática do serviço de pagamento da NexaOrder como demonstração central.
...
Isso significa que, em uma arquitetura com mTLS bem configurada, o serviço de pagamento autentica a identidade apresentada pelo chamador. Uma política de autorização separada decide se essa identidade pode executar a operação solicitada. Assim, estar na mesma rede não basta, mas possuir um certificado válido também não concede, por si só, permissão para reembolso ou cobrança.

> **Recurso visual 12 — Comunicação sem e com mTLS:** dois diagramas comparando uma chamada interna em texto claro sem verificação de identidade e uma chamada protegida por TLS mútuo com certificados em ambas as pontas.  
...
> **Recurso visual 13 — Service mesh com proxies laterais:** diagrama dos quatro serviços da NexaOrder, cada um acompanhado de um proxy lateral, todos coordenados por um plano de controle central que distribui certificados e políticas.  
...
> **Recurso visual 14 — Balde de fichas:** ilustração de um balde com fichas sendo reposto a taxa constante e requisições consumindo fichas; uma rajada acima da capacidade é parcialmente recusada.  
...
- **Ataque de repetição (*replay*):** uma mensagem legítima capturada é reenviada posteriormente para produzir um efeito indevido; mitigado pela combinação de identificador único persistido, janela de validade, verificação de integridade/autenticidade e rejeição atômica de identificadores já consumidos.
...
> **Recurso visual 15 — Fluxo autenticado pedido → pagamento:** diagrama passo a passo com verificação de certificado mútuo, checagem de autorização por identidade, aplicação de limite de taxa e verificação de idempotência antes do processamento.  
...
O roteiro falado completo, com narração pronta para gravação, marcações de edição e fontes, está no arquivo `roteiros_20min.md` desta unidade, usando o fluxo autenticado entre pedidos e pagamento da NexaOrder como demonstração central.
...
**Texto provocativo:** A Aula 9 mostrou que dividir um sistema em processos não produz autonomia por si só. Este livro é a referência que sustenta esse argumento: os capítulos iniciais tratam da modelagem de limites de serviço, e o capítulo sobre decomposição de um monólito existente descreve, passo a passo, como sair de um banco compartilhado sem interromper a operação — o problema exato enfrentado pela NexaOrder.

**Referência:** NEWMAN, Sam. *Building Microservices*. 2. ed. Sebastopol: O'Reilly Media, 2021. Capítulos iniciais sobre limites de serviço e capítulo sobre decomposição de monólitos.

**Link de acesso:** disponível na Biblioteca Virtual da instituição.

**Aula indicada:** Aula 9, após "Contexto delimitado e capacidade de negócio".

#### Para Mergulhar no Assunto

**Texto provocativo:** Este é o artigo que popularizou o termo "microsserviços" e, mais importante, o conjunto de características que o define — componentização por serviços, organização por capacidade de negócio, governança e dados descentralizados. Vale ler observando quantas dessas características a arquitetura em que você trabalha realmente possui, e quantas existem apenas no nome.

**Referência:** LEWIS, James; FOWLER, Martin. Microservices. *martinfowler.com*, 25 mar. 2014.

**Link de acesso:** <https://martinfowler.com/articles/microservices.html>. Acesso em: 30 jul. 2026.

**Aula indicada:** Aula 9, antes da atividade prática.

#### Podcast

**Texto provocativo:** Duas das principais referências da área conversam sobre a pergunta que esta unidade coloca desde o início: quando microsserviços realmente compensam — e quando eles apenas transferem complexidade do código para a operação. A conversa é especialmente útil para quem precisa justificar (ou recusar) uma migração diante de uma liderança técnica.

**Referência:** GOTO CONFERENCES. *When to use microservices (and when not!): Sam Newman & Martin Fowler*. [S. l.: s. n.], 2020. 1 vídeo. Publicado no YouTube.

**Link de acesso:** <https://www.youtube.com/watch?v=GBTdnfD6s5Q>. Acesso em: 30 jul. 2026.

**Trecho obrigatório:** 00:00–30:00 (30 minutos), limitado à discussão sobre critérios de adoção e custo operacional.

**Aula indicada:** Aula 9, após o comparativo entre monólito modular e microsserviços.

#### Artigo científico

**Texto provocativo:** O laço de reconciliação da Aula 11 não foi inventado do zero. Este artigo, escrito por engenheiros do Google, reconstrói dez anos de operação de contêineres em escala — do Borg ao Omega e ao Kubernetes — e explica quais lições operacionais levaram ao modelo declarativo de estado desejado. É a leitura que transforma "o Kubernetes recriou o Pod" em uma decisão de projeto compreensível.

**Referência:** BURNS, Brendan et al. Borg, Omega, and Kubernetes. *Communications of the ACM*, v. 59, n. 5, p. 50-57, 2016. DOI: 10.1145/2890784.

**Link de acesso:** <https://doi.org/10.1145/2890784>. Acesso em: 30 jul. 2026.

**Aula indicada:** Aula 11, após "Controladores e o laço de reconciliação".
