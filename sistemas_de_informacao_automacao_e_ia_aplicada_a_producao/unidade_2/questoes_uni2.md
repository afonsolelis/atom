# Questionário — Unidade 2

- **Disciplina:** Sistemas de Informação, Automação e IA Aplicada à Produção
- **Professor-conteudista:** Afonso Cesar Lelis Brandão

<!--
TABELA PARA USO EXCLUSIVO DO(A) COORDENADOR(A)

| Requisitos para um conteúdo UAU                                 | Observações |
| --------------------------------------------------------------- | ----------- |
| Conformidade com o Plano de Aprendizado                         |             |
| Conformidade com o conteúdo desenvolvido na unidade relacionada |             |
| Ortografia e gramática                                          |             |
| Questões padrão ENADE                                           |             |

| Data da Validação | Nome do(a) Coordenador(a) do curso |
| ----------------- | ---------------------------------- |
|                   |                                    |
-->

## Orientações

- **20 questões** padrão ENADE: **10 asserção-razão** + **10 de interpretação**.
- Cada questão tem **5 alternativas (a–e)**; a correta é prefixada por `*` (ex.: `*c. ...`).
- Para cada alternativa, escreva um **feedback explicativo** (por que está certa ou errada).
- Feedbacks ao final do documento, na ordem das questões.
- Pode-se aproveitar questões já aplicadas no ENADE, adaptadas ao curso.

---

## Questões

### Questão 1 (Asserção-Razão)

**Enunciado:**

_(insira a asserção e a razão, conectadas por "porque"/"pois")_

> Asserção I: _(afirmação principal)_
>
> **porque**
>
> Razão II: _(justificativa)_

A respeito dessas asserções, assinale a opção correta:

a. As asserções I e II são proposições verdadeiras, e a II é uma justificativa correta da I.
b. As asserções I e II são proposições verdadeiras, mas a II não é uma justificativa correta da I.
c. A asserção I é uma proposição verdadeira, e a II é uma proposição falsa.
d. A asserção I é uma proposição falsa, e a II é uma proposição verdadeira.
e. As asserções I e II são proposições falsas.

> **Marcar a correta com `*`** na linha da alternativa correta (ex.: `*a. ...`).
### Questão 2 (Asserção-Razão)

**Enunciado:**

_(insira a asserção e a razão, conectadas por "porque"/"pois")_

> Asserção I: _(afirmação principal)_
>
> **porque**
>
> Razão II: _(justificativa)_

A respeito dessas asserções, assinale a opção correta:

a. As asserções I e II são proposições verdadeiras, e a II é uma justificativa correta da I.
b. As asserções I e II são proposições verdadeiras, mas a II não é uma justificativa correta da I.
c. A asserção I é uma proposição verdadeira, e a II é uma proposição falsa.
d. A asserção I é uma proposição falsa, e a II é uma proposição verdadeira.
e. As asserções I e II são proposições falsas.

> **Marcar a correta com `*`** na linha da alternativa correta (ex.: `*a. ...`).
### Questão 3 (Asserção-Razão)

**Enunciado:**

_(insira a asserção e a razão, conectadas por "porque"/"pois")_

> Asserção I: _(afirmação principal)_
>
> **porque**
>
> Razão II: _(justificativa)_

A respeito dessas asserções, assinale a opção correta:

a. As asserções I e II são proposições verdadeiras, e a II é uma justificativa correta da I.
b. As asserções I e II são proposições verdadeiras, mas a II não é uma justificativa correta da I.
c. A asserção I é uma proposição verdadeira, e a II é uma proposição falsa.
d. A asserção I é uma proposição falsa, e a II é uma proposição verdadeira.
e. As asserções I e II são proposições falsas.

> **Marcar a correta com `*`** na linha da alternativa correta (ex.: `*a. ...`).
### Questão 4 (Asserção-Razão)

**Enunciado:**

_(insira a asserção e a razão, conectadas por "porque"/"pois")_

> Asserção I: _(afirmação principal)_
>
> **porque**
>
> Razão II: _(justificativa)_

A respeito dessas asserções, assinale a opção correta:

a. As asserções I e II são proposições verdadeiras, e a II é uma justificativa correta da I.
b. As asserções I e II são proposições verdadeiras, mas a II não é uma justificativa correta da I.
c. A asserção I é uma proposição verdadeira, e a II é uma proposição falsa.
d. A asserção I é uma proposição falsa, e a II é uma proposição verdadeira.
e. As asserções I e II são proposições falsas.

> **Marcar a correta com `*`** na linha da alternativa correta (ex.: `*a. ...`).
### Questão 5 (Asserção-Razão)

**Enunciado:**

_(insira a asserção e a razão, conectadas por "porque"/"pois")_

> Asserção I: _(afirmação principal)_
>
> **porque**
>
> Razão II: _(justificativa)_

A respeito dessas asserções, assinale a opção correta:

a. As asserções I e II são proposições verdadeiras, e a II é uma justificativa correta da I.
b. As asserções I e II são proposições verdadeiras, mas a II não é uma justificativa correta da I.
c. A asserção I é uma proposição verdadeira, e a II é uma proposição falsa.
d. A asserção I é uma proposição falsa, e a II é uma proposição verdadeira.
e. As asserções I e II são proposições falsas.

> **Marcar a correta com `*`** na linha da alternativa correta (ex.: `*a. ...`).
### Questão 6 (Asserção-Razão)

**Enunciado:**

_(insira a asserção e a razão, conectadas por "porque"/"pois")_

> Asserção I: _(afirmação principal)_
>
> **porque**
>
> Razão II: _(justificativa)_

A respeito dessas asserções, assinale a opção correta:

a. As asserções I e II são proposições verdadeiras, e a II é uma justificativa correta da I.
b. As asserções I e II são proposições verdadeiras, mas a II não é uma justificativa correta da I.
c. A asserção I é uma proposição verdadeira, e a II é uma proposição falsa.
d. A asserção I é uma proposição falsa, e a II é uma proposição verdadeira.
e. As asserções I e II são proposições falsas.

> **Marcar a correta com `*`** na linha da alternativa correta (ex.: `*a. ...`).
### Questão 7 (Asserção-Razão)

**Enunciado:**

_(insira a asserção e a razão, conectadas por "porque"/"pois")_

> Asserção I: _(afirmação principal)_
>
> **porque**
>
> Razão II: _(justificativa)_

A respeito dessas asserções, assinale a opção correta:

a. As asserções I e II são proposições verdadeiras, e a II é uma justificativa correta da I.
b. As asserções I e II são proposições verdadeiras, mas a II não é uma justificativa correta da I.
c. A asserção I é uma proposição verdadeira, e a II é uma proposição falsa.
d. A asserção I é uma proposição falsa, e a II é uma proposição verdadeira.
e. As asserções I e II são proposições falsas.

> **Marcar a correta com `*`** na linha da alternativa correta (ex.: `*a. ...`).
### Questão 8 (Asserção-Razão)

**Enunciado:**

_(insira a asserção e a razão, conectadas por "porque"/"pois")_

> Asserção I: _(afirmação principal)_
>
> **porque**
>
> Razão II: _(justificativa)_

A respeito dessas asserções, assinale a opção correta:

a. As asserções I e II são proposições verdadeiras, e a II é uma justificativa correta da I.
b. As asserções I e II são proposições verdadeiras, mas a II não é uma justificativa correta da I.
c. A asserção I é uma proposição verdadeira, e a II é uma proposição falsa.
d. A asserção I é uma proposição falsa, e a II é uma proposição verdadeira.
e. As asserções I e II são proposições falsas.

> **Marcar a correta com `*`** na linha da alternativa correta (ex.: `*a. ...`).
### Questão 9 (Asserção-Razão)

**Enunciado:**

_(insira a asserção e a razão, conectadas por "porque"/"pois")_

> Asserção I: _(afirmação principal)_
>
> **porque**
>
> Razão II: _(justificativa)_

A respeito dessas asserções, assinale a opção correta:

a. As asserções I e II são proposições verdadeiras, e a II é uma justificativa correta da I.
b. As asserções I e II são proposições verdadeiras, mas a II não é uma justificativa correta da I.
c. A asserção I é uma proposição verdadeira, e a II é uma proposição falsa.
d. A asserção I é uma proposição falsa, e a II é uma proposição verdadeira.
e. As asserções I e II são proposições falsas.

> **Marcar a correta com `*`** na linha da alternativa correta (ex.: `*a. ...`).
### Questão 10 (Asserção-Razão)

**Enunciado:**

_(insira a asserção e a razão, conectadas por "porque"/"pois")_

> Asserção I: _(afirmação principal)_
>
> **porque**
>
> Razão II: _(justificativa)_

A respeito dessas asserções, assinale a opção correta:

a. As asserções I e II são proposições verdadeiras, e a II é uma justificativa correta da I.
b. As asserções I e II são proposições verdadeiras, mas a II não é uma justificativa correta da I.
c. A asserção I é uma proposição verdadeira, e a II é uma proposição falsa.
d. A asserção I é uma proposição falsa, e a II é uma proposição verdadeira.
e. As asserções I e II são proposições falsas.

> **Marcar a correta com `*`** na linha da alternativa correta (ex.: `*a. ...`).
### Questão 11 (Interpretação)

**Estímulo:** _(texto, imagem, gráfico, charge, mapa ou outro material a interpretar — inclua a fonte)_

**Enunciado:** _(comando claro de ação que articule o estímulo a um único desafio)_

a. _(alternativa A)_
b. _(alternativa B)_
c. _(alternativa C)_
d. _(alternativa D)_
e. _(alternativa E)_

> **Marcar a correta com `*`** na linha da alternativa correta.
### Questão 12 (Interpretação)

**Estímulo:** _(texto, imagem, gráfico, charge, mapa ou outro material a interpretar — inclua a fonte)_

**Enunciado:** _(comando claro de ação que articule o estímulo a um único desafio)_

a. _(alternativa A)_
b. _(alternativa B)_
c. _(alternativa C)_
d. _(alternativa D)_
e. _(alternativa E)_

> **Marcar a correta com `*`** na linha da alternativa correta.
### Questão 13 (Interpretação)

**Estímulo:** _(texto, imagem, gráfico, charge, mapa ou outro material a interpretar — inclua a fonte)_

**Enunciado:** _(comando claro de ação que articule o estímulo a um único desafio)_

a. _(alternativa A)_
b. _(alternativa B)_
c. _(alternativa C)_
d. _(alternativa D)_
e. _(alternativa E)_

> **Marcar a correta com `*`** na linha da alternativa correta.
### Questão 14 (Interpretação)

**Estímulo:** _(texto, imagem, gráfico, charge, mapa ou outro material a interpretar — inclua a fonte)_

**Enunciado:** _(comando claro de ação que articule o estímulo a um único desafio)_

a. _(alternativa A)_
b. _(alternativa B)_
c. _(alternativa C)_
d. _(alternativa D)_
e. _(alternativa E)_

> **Marcar a correta com `*`** na linha da alternativa correta.
### Questão 15 (Interpretação)

**Estímulo:** _(texto, imagem, gráfico, charge, mapa ou outro material a interpretar — inclua a fonte)_

**Enunciado:** _(comando claro de ação que articule o estímulo a um único desafio)_

a. _(alternativa A)_
b. _(alternativa B)_
c. _(alternativa C)_
d. _(alternativa D)_
e. _(alternativa E)_

> **Marcar a correta com `*`** na linha da alternativa correta.
### Questão 16 (Interpretação)

**Estímulo:** _(texto, imagem, gráfico, charge, mapa ou outro material a interpretar — inclua a fonte)_

**Enunciado:** _(comando claro de ação que articule o estímulo a um único desafio)_

a. _(alternativa A)_
b. _(alternativa B)_
c. _(alternativa C)_
d. _(alternativa D)_
e. _(alternativa E)_

> **Marcar a correta com `*`** na linha da alternativa correta.
### Questão 17 (Interpretação)

**Estímulo:** _(texto, imagem, gráfico, charge, mapa ou outro material a interpretar — inclua a fonte)_

**Enunciado:** _(comando claro de ação que articule o estímulo a um único desafio)_

a. _(alternativa A)_
b. _(alternativa B)_
c. _(alternativa C)_
d. _(alternativa D)_
e. _(alternativa E)_

> **Marcar a correta com `*`** na linha da alternativa correta.
### Questão 18 (Interpretação)

**Estímulo:** _(texto, imagem, gráfico, charge, mapa ou outro material a interpretar — inclua a fonte)_

**Enunciado:** _(comando claro de ação que articule o estímulo a um único desafio)_

a. _(alternativa A)_
b. _(alternativa B)_
c. _(alternativa C)_
d. _(alternativa D)_
e. _(alternativa E)_

> **Marcar a correta com `*`** na linha da alternativa correta.
### Questão 19 (Interpretação)

**Estímulo:** _(texto, imagem, gráfico, charge, mapa ou outro material a interpretar — inclua a fonte)_

**Enunciado:** _(comando claro de ação que articule o estímulo a um único desafio)_

a. _(alternativa A)_
b. _(alternativa B)_
c. _(alternativa C)_
d. _(alternativa D)_
e. _(alternativa E)_

> **Marcar a correta com `*`** na linha da alternativa correta.
### Questão 20 (Interpretação)

**Estímulo:** _(texto, imagem, gráfico, charge, mapa ou outro material a interpretar — inclua a fonte)_

**Enunciado:** _(comando claro de ação que articule o estímulo a um único desafio)_

a. _(alternativa A)_
b. _(alternativa B)_
c. _(alternativa C)_
d. _(alternativa D)_
e. _(alternativa E)_

> **Marcar a correta com `*`** na linha da alternativa correta.


---

## Feedbacks

### Questão 1

- **a.** _(feedback explicativo — se correta, explique o conceito; se incorreta, explique o erro e aponte a correta)_
- **b.** _(feedback)_
- **c.** _(feedback)_
- **d.** _(feedback)_
- **e.** _(feedback)_
### Questão 2

- **a.** _(feedback explicativo — se correta, explique o conceito; se incorreta, explique o erro e aponte a correta)_
- **b.** _(feedback)_
- **c.** _(feedback)_
- **d.** _(feedback)_
- **e.** _(feedback)_
### Questão 3

- **a.** _(feedback explicativo — se correta, explique o conceito; se incorreta, explique o erro e aponte a correta)_
- **b.** _(feedback)_
- **c.** _(feedback)_
- **d.** _(feedback)_
- **e.** _(feedback)_
### Questão 4

- **a.** _(feedback explicativo — se correta, explique o conceito; se incorreta, explique o erro e aponte a correta)_
- **b.** _(feedback)_
- **c.** _(feedback)_
- **d.** _(feedback)_
- **e.** _(feedback)_
### Questão 5

- **a.** _(feedback explicativo — se correta, explique o conceito; se incorreta, explique o erro e aponte a correta)_
- **b.** _(feedback)_
- **c.** _(feedback)_
- **d.** _(feedback)_
- **e.** _(feedback)_
### Questão 6

- **a.** _(feedback explicativo — se correta, explique o conceito; se incorreta, explique o erro e aponte a correta)_
- **b.** _(feedback)_
- **c.** _(feedback)_
- **d.** _(feedback)_
- **e.** _(feedback)_
### Questão 7

- **a.** _(feedback explicativo — se correta, explique o conceito; se incorreta, explique o erro e aponte a correta)_
- **b.** _(feedback)_
- **c.** _(feedback)_
- **d.** _(feedback)_
- **e.** _(feedback)_
### Questão 8

- **a.** _(feedback explicativo — se correta, explique o conceito; se incorreta, explique o erro e aponte a correta)_
- **b.** _(feedback)_
- **c.** _(feedback)_
- **d.** _(feedback)_
- **e.** _(feedback)_
### Questão 9

- **a.** _(feedback explicativo — se correta, explique o conceito; se incorreta, explique o erro e aponte a correta)_
- **b.** _(feedback)_
- **c.** _(feedback)_
- **d.** _(feedback)_
- **e.** _(feedback)_
### Questão 10

- **a.** _(feedback explicativo — se correta, explique o conceito; se incorreta, explique o erro e aponte a correta)_
- **b.** _(feedback)_
- **c.** _(feedback)_
- **d.** _(feedback)_
- **e.** _(feedback)_
### Questão 11

- **a.** _(feedback explicativo — se correta, explique o conceito; se incorreta, explique o erro e aponte a correta)_
- **b.** _(feedback)_
- **c.** _(feedback)_
- **d.** _(feedback)_
- **e.** _(feedback)_
### Questão 12

- **a.** _(feedback explicativo — se correta, explique o conceito; se incorreta, explique o erro e aponte a correta)_
- **b.** _(feedback)_
- **c.** _(feedback)_
- **d.** _(feedback)_
- **e.** _(feedback)_
### Questão 13

- **a.** _(feedback explicativo — se correta, explique o conceito; se incorreta, explique o erro e aponte a correta)_
- **b.** _(feedback)_
- **c.** _(feedback)_
- **d.** _(feedback)_
- **e.** _(feedback)_
### Questão 14

- **a.** _(feedback explicativo — se correta, explique o conceito; se incorreta, explique o erro e aponte a correta)_
- **b.** _(feedback)_
- **c.** _(feedback)_
- **d.** _(feedback)_
- **e.** _(feedback)_
### Questão 15

- **a.** _(feedback explicativo — se correta, explique o conceito; se incorreta, explique o erro e aponte a correta)_
- **b.** _(feedback)_
- **c.** _(feedback)_
- **d.** _(feedback)_
- **e.** _(feedback)_
### Questão 16

- **a.** _(feedback explicativo — se correta, explique o conceito; se incorreta, explique o erro e aponte a correta)_
- **b.** _(feedback)_
- **c.** _(feedback)_
- **d.** _(feedback)_
- **e.** _(feedback)_
### Questão 17

- **a.** _(feedback explicativo — se correta, explique o conceito; se incorreta, explique o erro e aponte a correta)_
- **b.** _(feedback)_
- **c.** _(feedback)_
- **d.** _(feedback)_
- **e.** _(feedback)_
### Questão 18

- **a.** _(feedback explicativo — se correta, explique o conceito; se incorreta, explique o erro e aponte a correta)_
- **b.** _(feedback)_
- **c.** _(feedback)_
- **d.** _(feedback)_
- **e.** _(feedback)_
### Questão 19

- **a.** _(feedback explicativo — se correta, explique o conceito; se incorreta, explique o erro e aponte a correta)_
- **b.** _(feedback)_
- **c.** _(feedback)_
- **d.** _(feedback)_
- **e.** _(feedback)_
### Questão 20

- **a.** _(feedback explicativo — se correta, explique o conceito; se incorreta, explique o erro e aponte a correta)_
- **b.** _(feedback)_
- **c.** _(feedback)_
- **d.** _(feedback)_
- **e.** _(feedback)_

