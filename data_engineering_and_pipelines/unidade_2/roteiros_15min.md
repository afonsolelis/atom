...
> **Preparação da unidade:** o Codespace deve estar com a Unidade 1 concluída (`data/raw` com os 9 CSVs, `olist.duckdb` com o schema `raw` carregado). Para a Aula 6, rode `pip install pyspark` **antes** de gravar (instalação demorada). Para a Aula 8, instale o Airflow antes (comando na própria aula) e grave com ele já instalado.

---

## Roteiro da Videoaula 5 — "ETL vs ELT: nasce o projeto dbt do Olist"
...
> "Olá! Bem-vindo, bem-vinda à Unidade 2 — a unidade em que o dado do Olist entra **em movimento**. Na Unidade 1, a gente carregou os 9 CSVs no schema `raw` com scripts soltos de Python. Funciona **uma vez** — mas não é um pipeline: não renomeia coluna, não casta tipo, não trata nulo e, pior, recarrega os 99 mil pedidos toda santa execução. Hoje a gente transforma aquilo numa **ingestão de verdade**: nasce o projeto **`dbt_olist`**, no padrão **ELT**, com modelos staging, carga **incremental** — e a palavra que vai te salvar muitas madrugadas na carreira: **idempotência**. E tudo digitado e rodado ao vivo aqui no Codespace. Bora."
...
> "O ELT venceu porque armazenar ficou barato — os 120 megabytes crus do Olist custam frações de centavo, fizemos essa conta na Aula 1 — e porque os motores ficaram poderosos. Carregar o bruto primeiro significa **guardar a fonte da verdade**: errou a transformação? Re-roda `dbt run`. Sem reextração, sem drama. E guarda a frase-mantra do curso: **o que roda local com DuckDB e dbt migra para a nuvem trocando só o profile — os conceitos são os mesmos**."

### 3. Mão na massa: criando o projeto dbt (3:45 – 7:15)

**[TELA]** Terminal do Codespace.

> "Bora criar o projeto. O **dbt** — *data build tool* — é a ferramenta que transforma SQL solto em projeto de engenharia: versionado, testável, documentado. Já instalamos o `dbt-duckdb` lá na Aula 1; agora é usar."
...
**[CÓDIGO]** Criar `dbt_olist/profiles.yml` (o dbt procura primeiro na pasta do projeto):
...
> "`All checks passed!` — o dbt encontrou o projeto, o profile e conectou no nosso `olist.duckdb`. Repara na elegância: o `profiles.yml` diz apenas 'tipo duckdb, caminho do arquivo'. Se um dia esse projeto for para o BigQuery ou o Snowflake, **só esse arquivo muda** — os modelos SQL ficam idênticos. É a portabilidade que eu prometi."

> "Agora, o primeiro conceito de organização do dbt: os **sources**. A gente declara as tabelas do schema `raw` como fontes nomeadas — assim o dbt conhece a origem, consegue testá-la e desenha a **linhagem** do dado."
...
> "Repara na sintaxe do `stg_orders`, porque cada pedaço conta uma história. O `config`: **materialized incremental** com **unique_key order_id** — já explico o poder disso. O corpo: renomeia `order_purchase_timestamp` para `purchased_at` e **casta** para timestamp — chega de string fingindo ser data. E o bloco `is_incremental()`: **na primeira execução ele é ignorado** e a carga é full; **das próximas em diante**, só entram pedidos **mais novos** que o máximo já carregado. Um modelo, dois comportamentos, zero código duplicado."
...
> "Olha o log: três modelos, três `OK` — `stg_orders` criado como **incremental**, os outros como view. O dado do Olist agora está limpo, tipado e renomeado dentro do banco. E agora, o teste que separa pipeline amador de pipeline profissional. Pergunta: o que acontece se eu rodar **de novo**?"
...
> "Olá! Bem-vindo, bem-vinda de volta. O dado do Olist está limpo nos modelos staging. Agora, a pergunta de negócio: **qual o faturamento por categoria?** Responder exige **juntar** itens com produtos e **agregar** sobre 112 mil linhas — isso é **processamento em lote**, e o nome mais famoso desse mundo é **Apache Spark**. Hoje você entende a ideia fundadora — o MapReduce do Google —, a arquitetura do Spark, por que ele é 'preguiçoso' e qual operação custa mais caro. E rodamos **os dois lado a lado no Codespace**: o mesmo job em PySpark e em DuckDB, com cronômetro. Bora."
...
### 4. Mão na massa: o job em PySpark (5:45 – 9:30)
...
> "Sua missão: rodar o job PySpark, usar o `.explain()` e **contar os Exchange** no plano — antes e depois do broadcast; e rodar o equivalente DuckDB com cronômetro, anotando a diferença. Sinta na pele o custo do overhead distribuído."

> "E na próxima aula, mudamos o eixo do tempo: até agora o Olist é um **histórico parado**. Mas e se os pedidos estivessem chegando **agora**, um a um — e a gente precisasse contar 'pedidos por minuto' em tempo real? Vamos **simular o stream do Olist** ao vivo: produtor, consumidor, janelas — e descobrir, nos dados, **qual foi o minuto mais movimentado da história do Olist**. Spoiler: envolve uma Black Friday. Te espero na Aula 7. Um abraço!"
...
### 4. Mão na massa: o stream do Olist ao vivo (6:00 – 10:30)
...
> "99.441 eventos processados — e olha o ranking das janelas! Os minutos mais movimentados da história do Olist estão concentrados em… **24 de novembro de 2017**. Que dia foi esse? **A Black Friday!** A gente acabou de descobrir, com um stream simulado e uma janela tumbling, o pico histórico do marketplace — dezenas de pedidos num único minuto, contra uma média de 135 **por dia**. Repara na anatomia do código: o `emit` é o produtor — na vida real seria um `producer.send` do Kafka; o `consume` recebe o evento, trunca o timestamp no minuto — esse é o *bucket* da **janela tumbling** — e incrementa. Trinta linhas, e todos os conceitos do streaming estão aí dentro."
...
> "Olá! Bem-vindo, bem-vinda à última aula da unidade — e à mais esperada. Você já sabe **ingerir** com dbt, **processar** em lote e **simular** um stream do Olist. Mas até agora rodamos cada passo **na mão**: `dbt run` num terminal, script no outro. Pipeline de verdade não depende de humano lembrando de apertar botão: tem tarefas com **dependências**, horários, tratamento de falha e prazo de entrega. Hoje a gente sobe o **Apache Airflow dentro do Codespaces**, abre a interface web no navegador e monta o DAG **`olist_pipeline`**: ingestão → dbt run → dbt test → export gold — o primeiro pipeline do Olist orquestrado **ponta a ponta**, rodando na tela. Bora."
...
### 3. Mão na massa: subindo o Airflow no Codespaces (3:30 – 6:30)

**[TELA]** Terminal (instalação feita antes de gravar; mostrar o comando e rodar o standalone ao vivo).
...
> "Olha o que o `standalone` faz por nós: inicializa o banco de metadados, cria o usuário **admin** — a senha está no log e no arquivo `airflow/standalone_admin_password.txt` — e sobe o **webserver na porta 8080**. E agora, o momento bonito: lembra que lá na **Aula 1** a gente configurou `forwardPorts: [8080]` no devcontainer? O Codespaces detecta a porta e me oferece **abrir no navegador**. Clico… e aqui está: a **interface web do Airflow**, rodando dentro do nosso Codespace, de graça. Nada naquela aula de setup foi por acaso. E repara também no `AIRFLOW_HOME` apontando para a nossa pasta `airflow/` — é por isso que os DAGs que criarmos em `airflow/dags/` aparecem direto na interface."
...
with DAG(
    ...
    dbt_run  = BashOperator(task_id="dbt_run",
                            bash_command=f"cd {BASE}/dbt_olist && dbt run")
    dbt_test = BashOperator(task_id="dbt_test",
                            bash_command=f"cd {BASE}/dbt_olist && dbt test")
