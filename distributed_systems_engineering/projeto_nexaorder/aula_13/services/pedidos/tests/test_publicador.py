...
def test_evento_publicado_carrega_o_trace_id_do_pedido(cliente_api):
    """Unidade 4, Aula 13 — o terceiro passo da propagação de correlação:
    o salto assíncrono também carrega o identificador, não só as chamadas
    HTTP síncronas. Sem isto, a jornada se rompe exatamente no ponto em
    que o pedido vira um evento (ver docs/observabilidade.md)."""
    import app.main as modulo_pedidos

    pedido = cliente_api.post("/pedidos", json=_corpo_pedido()).json()
    topico = Topico(NOME_TOPICO_PEDIDOS, NUM_PARTICOES_PEDIDOS)

    publicar_eventos_pendentes(modulo_pedidos.repositorio, topico)

    particao = topico.particao_da_chave(pedido["id"])
    evento_publicado = next(e for e in topico.ler_particao(particao) if e.chave == pedido["id"])

    assert evento_publicado.trace_id == pedido["trace_id"]
    assert evento_publicado.trace_id != ""
