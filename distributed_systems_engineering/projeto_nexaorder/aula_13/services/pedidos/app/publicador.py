...
def publicar_eventos_pendentes(repositorio: RepositorioPedidos, topico: Topico) -> list[str]:
    ...
    for evento in eventos:
        topico.publicar(
            ...
            # O trace_id já viaja dentro do payload desde a Aula 3
            # (ver store.py: `salvar` o grava no evento da outbox junto
            # com o pedido). A Aula 13 só o promove de campo do payload
            # para metadado de primeira classe do evento — o mesmo dado,
            # em um lugar em que qualquer consumidor sabe procurar sem
            # precisar conhecer o formato de cada tipo de payload.
            trace_id=evento["payload"].get("trace_id", ""),
        )
