...
from .eventos_dispositivo import NOME_TOPICO_TENTATIVAS, NUM_PARTICOES_TENTATIVAS, publicar_tentativa
from .janela_evento import JanelaPorTempoDeEvento
from .janela_evento import Tentativa as TentativaNaJanela
from .lamport import LamportClock
from .logs_estruturados import registrar as registrar_log
from .metricas import ContadorComRotulos
from .models import CriarPedidoRequest, EstadoPedido, PedidoResponse, TentativaDePagamentoRequest
...
app = FastAPI(title="NexaOrder — Pedidos", version="0.15.0")
...
# Aula 15 — pipeline de fluxo para detecção de fraude, particionado por
# dispositivo. A janela nunca lê o relógio por si (ver app/janela_evento.py);
# é este módulo, na borda real de entrada, quem converte time.time() em
# milissegundos ao ingerir uma tentativa.
topico_tentativas = Topico(nome=NOME_TOPICO_TENTATIVAS, num_particoes=NUM_PARTICOES_TENTATIVAS)
janela_fraude = JanelaPorTempoDeEvento(duracao_ms=60_000, tolerancia_atraso_ms=30_000)
...
@app.post("/_admin/fraude/tentativa", status_code=201)
async def registrar_tentativa_de_pagamento(corpo: TentativaDePagamentoRequest) -> dict[str, Any]:
    """Demonstra o pipeline de fluxo da Aula 15: publica a tentativa no
    tópico particionado por dispositivo e a ingere na janela por tempo de
    evento. `time.time()` só existe aqui, na borda de entrada — a lógica de
    janela em si (app/janela_evento.py) é pura e nunca lê o relógio."""
    agora_ms = int(time.time() * 1000)
    publicar_tentativa(topico_tentativas, corpo.dispositivo_id, {"tempo_evento_ms": corpo.tempo_evento_ms})
    janela_fraude.ingerir(
        TentativaNaJanela(
            dispositivo_id=corpo.dispositivo_id,
            tempo_evento_ms=corpo.tempo_evento_ms,
            tempo_processamento_ms=agora_ms,
        )
    )
    janela_fraude.avancar_marca_dagua(agora_ms)
    return {
        "aceito": True,
        "particao": topico_tentativas.particao_da_chave(corpo.dispositivo_id),
        "tempo_processamento_ms": agora_ms,
    }


@app.get("/_admin/fraude/contagem/{dispositivo_id}")
async def contagem_de_tentativas(dispositivo_id: str, fim_da_janela_ms: int) -> dict[str, Any]:
    """Contagem por tempo de EVENTO na janela — a base que a Aula 15
    recomenda para decidir, não o tempo de processamento (ver
    docs/processamento.md)."""
    return {
        "dispositivo_id": dispositivo_id,
        "fim_da_janela_ms": fim_da_janela_ms,
        "contagem_na_janela": janela_fraude.contagem_na_janela(dispositivo_id, fim_da_janela_ms),
        "janela_fechada": janela_fraude.esta_fechada(fim_da_janela_ms),
    }
